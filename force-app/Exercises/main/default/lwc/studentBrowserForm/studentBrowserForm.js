...
import getDeliveriesByInstructor from '@salesforce/apex/StudentBrowser.getDeliveriesByInstructor';
...
wired_getInstructors({ error, data }) {
    ...
    if (data) {
        ...
        data.forEach(instructor => {this.instructors.push({value: instructor.Id,label: instructor.Name});
    });
    } else if (error) {
    this.error = error;
    }
...
@wire(getDeliveriesByInstructor, { instructorId:'$selectedInstructorId' })
    wired_getDeliveriesByInstructor({ error, data }) {
        ...
        if (data && data.length) {
            ...
            this.deliveries.unshift({value: '',label: 'Any Delivery'});
        } 
        else if (error) {
            this.error = error;
        }
